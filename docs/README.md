...
## これは何ですか？
以下のデータ形式にjinja2テンプレートをテンプレートを適用する汎用的ツールです。
...
Excelは完全には対応できてませんが、ある程度の範囲で扱えます。
...
#### --dump ダンプ出力
このオプションを指定すると、toj2はjinja2を使用せずにjsonとして読込み結果を出力します。主にデバッグ用途です。
...
## jinja2カスタムフィルタ
toj2で扱うデータ形式をより扱いやすくするために、jinja2にカスタムフィルタを追加しています。

#### stable_groupby

jinja2組込みの`groupby`フィルタは対象の列をソートしてからグルーピングしますが、`stable_groupby`はソートを行わずにグルーピングを行います。csvなど、**行の出現順に意味がある場合**に効果を発揮します。
...
6,久米 ひろ子,茶道部
```
このcsvで部活によってグルーピングすると佐藤 はじめが1グループ、小林 裕太と宮田 敦が1グループになります。

jinja2組込みの`groupby`のような`default`などの機能は持っていません。

#### datef
使用例
```
    value | datef('%Y/%m/%d', '%Y-%m-%d')
```
日付形式の変換を行います。左側の値が不正な日付の場合は何もしません。

#### csvcell
出力をcsvカラムとしてエスケープします。例えば文字列`va"lue`は`"va""lue"`のように、一般的なcsvパーサが読み込める形式になります。
